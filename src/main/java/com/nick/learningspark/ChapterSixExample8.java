...
public class ChapterSixExample8 {
	...
	public static void main(String[] args) throws Exception {
		...
		/*
		 * rdd.foreach(new VoidFunction<String>() { public void call(String
		 * line) { if (line.contains("KK6JKQ")) { count.add(1); } } });
		 */

		// nick
		rdd.foreach(line -> line.contains("KK6JKQ"));
		...
		/*
		 * JavaRDD<String> callSigns = rdd .flatMap(new FlatMapFunction<String,
		 * String>() { public Iterable<String> call(String line) { if
		 * (line.equals("")) { blankLines.add(1); } return
		 * Arrays.asList(line.split(" ")); } });
		 */

		// nick
		JavaRDD<String> callSigns = rdd.flatMap(lines -> {
			if (lines.equals("")) {
				blankLines.add(1);
			}
		...
		/*
		 * JavaRDD<String> validCallSigns = callSigns .filter(new
		 * Function<String, Boolean>() { public Boolean call(String callSign) {
		 * Pattern p = Pattern
		 * .compile("\\A\\d?\\p{Alpha}{1,2}\\d{1,4}\\p{Alpha}{1,3}\\Z"); Matcher
		 * m = p.matcher(callSign); boolean b = m.matches(); if (b) {
		 * validSignCount.add(1); } else { invalidSignCount.add(1); } return b;
		 * } });
		 */
		...
		/*
		 * JavaPairRDD<String, Integer> contactCounts =
		 * validCallSigns.mapToPair( new PairFunction<String, String, Integer>()
		 * { public Tuple2<String, Integer> call(String callSign) { return new
		 * Tuple2(callSign, 1); } }).reduceByKey(new Function2<Integer, Integer,
		 * Integer>() { public Integer call(Integer x, Integer y) { return x +
		 * y; } });
		 */

		// nick
		JavaPairRDD<String, Integer> contactCounts = validCallSigns.mapToPair(
				callSign -> new Tuple2<String, Integer>(callSign, 1))
				.reduceByKey((x, y) -> x + y);
		
		Map<String, Object> contactCounts1 = validCallSigns.mapToPair(
				callSign -> new Tuple2<String, Integer>(callSign, 1))
				.countByKey();
		...
		JavaPairRDD<String, Integer> countryContactCounts = contactCounts.mapToPair(
				(Tuple2<String, Integer> callSignCount) -> 
				new Tuple2<String, Integer>(lookupCountry(callSignCount._1(), signPrefixes.value()), 
						callSignCount._2()));				
		...
		//nick
		JavaPairRDD<String, CallLog[]> contactsContactLists = validCallSigns.mapPartitionsToPair( (Iterator<String> input) -> {
			ArrayList<Tuple2<String, CallLog[]>> callsignQsos = new ArrayList<Tuple2<String, CallLog[]>>();
			ArrayList<Tuple2<String, ContentExchange>> requests = new ArrayList<Tuple2<String, ContentExchange>>();
			ObjectMapper mapper = createMapper();
			HttpClient client = new HttpClient();
