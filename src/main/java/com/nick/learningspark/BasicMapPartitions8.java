...
public class BasicMapPartitions8 {
	...
	public static void main(String[] args) throws Exception {
		...
		JavaSparkContext sc = new JavaSparkContext(master,
				"basicmappartitions", System.getenv("SPARK_HOME"),
				System.getenv("JARS"));

		JavaRDD<String> rdd = sc.parallelize(Arrays.asList("KK6JKQ", "Ve3UoW",
				"kk6jlk", "W6BB"));
						...
						try {
							...
							while (input.hasNext()) {
								ContentExchange exchange = new ContentExchange(
										true);
								exchange.setURL("http://qrzcq.com/call/"
										+ input.next());
		...
		//nick
		JavaRDD<String> result = rdd.mapPartitions((Iterator<String> input) -> {
				
			ArrayList<String> content = new ArrayList<String>();
			ArrayList<ContentExchange> contentExchange= new ArrayList<ContentExchange>();
			HttpClient client = new HttpClient();
			try{
				client.start();
			...
			while(input.hasNext()){
				...
				client.send(exchange);
				contentExchange.add(exchange);
				
				for(ContentExchange ce : contentExchange){
					ce.waitForDone();
					content.add(ce.getResponseContent());
