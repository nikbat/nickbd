...
public final class IntersectByKey8 {
	
	
	/*public static <K, V> JavaPairRDD<K, V> intersectByKey(
			JavaPairRDD<K, V> rdd1, JavaPairRDD<K, V> rdd2) {
		JavaPairRDD<K, Tuple2<Iterable<V>, Iterable<V>>> grouped = rdd1
				.cogroup(rdd2);
		return grouped
				.flatMapValues(new Function<Tuple2<Iterable<V>, Iterable<V>>, Iterable<V>>() {
					@Override
					public Iterable<V> call(
							Tuple2<Iterable<V>, Iterable<V>> input) {
						ArrayList<V> al = new ArrayList<V>();
						if (!Iterables.isEmpty(input._1())
								&& !Iterables.isEmpty(input._2())) {
	...
	public static void main(String[] args) throws Exception {
		...
		JavaSparkContext sc = new JavaSparkContext(master, "IntersectByKey",
		...
		//nick
		JavaPairRDD<String, Integer> result = rdd1.cogroup(rdd2).flatMapValues( (Tuple2<Iterable<Integer>, Iterable<Integer>> input) -> {
